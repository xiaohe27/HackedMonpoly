  ...
  if !displayver then
	...
	if !sigout then
	  Predicate.print_vartypes_list vartypes
	      ...
	      (* start counting number of log entries*)
              let numOfLogEntries= Algorithm.countLogEntries !logfile in
		print_string ("There are " ^ (string_of_int numOfLogEntries) ^ " log entries in the log file!")
        end
      end
